...
class TestVeryLongTermKeplerian:
    ...
    def test_leo_90day_keplerian_stability(self):
        ...
        energy_0 = (np.linalg.norm(v0)**2 / 2.0 -
                   constants.GM_EARTH / np.linalg.norm(r0))
        h_0 = np.cross(r0, v0)
        ...
        energy_final = (np.linalg.norm(v_final)**2 / 2.0 -
                       constants.GM_EARTH / np.linalg.norm(r_final))
        ...
        assert energy_rel_error < 1e-9, \
            f"Energy error after 90 days: {energy_rel_error:.2e}"

        assert h_rel_error < 1e-9, \
            f"Angular momentum error after 90 days: {h_rel_error:.2e}"

        # Verify orbital elements are preserved
        elements_final = rv_to_coe(r_final, v_final, constants.GM_EARTH)

        assert abs(elements_final.a - a) / a < 1e-8, \
            f"Semi-major axis drift: {abs(elements_final.a - a):.3f} m"

        assert abs(elements_final.e - e) < 1e-8, \
    ...
    def test_geo_180day_keplerian_stability(self):
        ...
        # Final state
        energy_final = np.linalg.norm(v_final)**2 / 2.0 - constants.GM_EARTH / np.linalg.norm(r_final)
        h_final = np.cross(r_final, v_final)
        h_mag_final = np.linalg.norm(h_final)
        ...
        assert energy_rel_error < 1e-9, \
            f"GEO energy error after 180 days: {energy_rel_error:.2e}"

        assert h_rel_error < 1e-9, \
            f"GEO angular momentum error after 180 days: {h_rel_error:.2e}"
    ...
    def test_molniya_365day_keplerian_stability(self):
        ...
        assert energy_rel_error < 1e-8, \
            f"Molniya energy error after 1 year: {energy_rel_error:.2e}"

        assert h_rel_error < 1e-8, \
            f"Molniya angular momentum error after 1 year: {h_rel_error:.2e}"

        # Verify orbital elements
        elements_final = rv_to_coe(r_final, v_final, constants.GM_EARTH)

        assert abs(elements_final.a - a) / a < 1e-7, \
            "Semi-major axis not stable over 1 year"

        assert abs(elements_final.e - e) < 1e-7, \
            "Eccentricity not stable over 1 year"
...
class TestIntegratorComparison:
    ...
    def test_j2_integrators_leo_90days(self):
        ...
        assert 6000e3 < r_dop_mag < 20e6, \
            f"DOP853 orbit seems unrealistic: r = {r_dop_mag/1e3:.1f} km"

        assert 6000e3 < r_dopri5_mag < 20e6, \
        ...
        if r_rk4_mag > 20e6:
            ...
            print(f"      This demonstrates why adaptive integrators are essential for long-term propagation")
        else:
    ...
    def test_j2_integrator_stability_geo_180days(self):
        ...
        assert pos_diff < 50000.0, \
            f"DOPRI5 vs DOP853 at GEO after 180 days: {pos_diff:.3f} m"

        assert vel_diff < 50.0, \
        ...
        assert 40000e3 < r_mag_final < 45000e3, \
...
class TestPerturbedOrbitStability:
    ...
    def test_leo_j2_drag_90days(self):
        ...
        r_final, v_final = propagate_j2_drag_dopri5(
            r0, v0, dt, constants.GM_EARTH,
            constants.J2_EARTH, constants.R_EARTH,
            constants.RHO0_EARTH, constants.H0_EARTH,
            B,
            tol=1e-8
        )
        ...
        assert alt_loss > 0, \
            f"Expected altitude loss, got: {alt_loss/1e3:.3f} km"

        # At 400 km with A/m=0.01, expect several km of decay over 90 days
        assert 1e3 < alt_loss < 50e3, \
            f"Altitude loss of {alt_loss/1e3:.3f} km seems unrealistic"
    ...
    def test_geo_thirdbody_180days(self):
        ...
        assert elements_final.e < 0.1, \
            f"Eccentricity grew unreasonably: e = {elements_final.e:.4f}"
    ...
    def test_geo_srp_365days(self):
        ...
        r_final, v_final = propagate_srp_dopri5(
            r0, v0, dt, constants.GM_EARTH,
            A_over_m, C_r, constants.R_EARTH,
            t0=t0,
            tol=1e-8
        )
...
class TestNumericalDriftAnalysis:
    ...
    def test_leo_j2_longterm_drift(self):
        ...
        for i in range(n_intervals):
            r, v = propagate_j2_dopri5(
                r, v, dt_interval, constants.GM_EARTH,
                constants.J2_EARTH, constants.R_EARTH,
                tol=1e-9
            )
    ...
    def test_energy_drift_keplerian_vs_j2(self):
        ...
        assert kep_energy_error < 1e-9, \
            f"Keplerian energy not conserved: {kep_energy_error:.2e}"
        ...
        assert j2_energy_change < 0.1, \
            f"J2 energy change seems excessive: {j2_energy_change:.2e}"
...
class TestStepSizeSensitivity:
    ...
    def test_rk4_step_size_convergence(self):
        ...
        assert np.linalg.norm(r_ref) > 6000e3, \
            "Orbit should remain above Earth's surface"
