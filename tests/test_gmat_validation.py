...
class TestForwardBackwardReversibility:
    ...
    def test_molniya_reversibility_3days(self):
        ...
        assert vel_error < 0.01, \
            f"Molniya 3-day reversibility velocity error: {vel_error:.6f} m/s"
    ...
    def test_geo_reversibility_7days(self):
        ...
        assert vel_error < 0.001, \
            f"GEO 7-day reversibility velocity error: {vel_error:.6f} m/s"
...
class TestLongTermPropagationStability:
    ...
    def test_molniya_60day_stability(self):
        ...
        assert abs(elements_final.a - a) / a < 1e-8, \
            "Semi-major axis drifted over 60 days"

        assert abs(elements_final.e - e) < 1e-8, \
            "Eccentricity drifted over 60 days"

        assert abs(elements_final.i - i) < 1e-10, \
            "Inclination drifted over 60 days"
...
class TestPropagatorComparison:
    ...
    def test_keplerian_vs_j2_rk4_leo_1day(self):
        ...
        energy_kep = (np.linalg.norm(v_kep)**2 / 2.0 -
                     constants.GM_EARTH / np.linalg.norm(r_kep))
        energy_0 = (np.linalg.norm(v0)**2 / 2.0 -
                   constants.GM_EARTH / np.linalg.norm(r0))

        # Keplerian should conserve energy perfectly
        kep_energy_error = abs(energy_kep - energy_0) / abs(energy_0)
        assert kep_energy_error < 1e-10, \
...
class TestStandardOrbitRegimes:
    ...
    def test_leo_regime_coverage(self):
        ...
        for alt in test_altitudes:
            ...
            assert 200e3 < altitude_check < 2000e3, \
            ...
            assert pos_error < 1.0, \
                f"LEO {alt/1e3}km: 1-orbit position error = {pos_error:.3f} m"
    ...
    def test_meo_gps_orbit(self):
        ...
        assert rel_error < 1e-10, \
            f"GPS orbit energy conservation over 7 days: {rel_error:.2e}"
    ...
    def test_heo_tundra_orbit(self):
        ...
        elements = OrbitalElements(
            a=a_tundra, e=e_tundra, i=i_tundra,
            raan=0.0, argp=np.deg2rad(-90.0), nu=0.0
        )
        ...
        assert abs(elements_final.a - a_tundra) / a_tundra < 1e-8, \
            "Tundra semi-major axis not preserved"

        assert abs(elements_final.e - e_tundra) < 1e-8, \
            "Tundra eccentricity not preserved"
...
class TestOrbitTypeClassification:
    ...
    def test_elliptical_orbit_range(self):
        ...
        for e in test_eccentricities:
            elements = OrbitalElements(
                a=20000e3, e=e, i=np.deg2rad(30.0),
                raan=0.0, argp=0.0, nu=0.0
            )

            # Verify e is in elliptical range
            assert 0.0 < elements.e < 1.0, \
                f"Eccentricity {e} should be elliptical"
            ...
            assert energy < 0, \
                f"Elliptical orbit (e={e}) should have negative energy"
...
class TestPropagationPerformance:
    ...
    def test_very_long_propagation_leo(self):
        ...
        """
        a = constants.R_EARTH + 500e3
        elements = OrbitalElements(a=a, e=0.001, i=np.deg2rad(51.6),
                                   raan=0.0, argp=0.0, nu=0.0)
        ...
        assert rel_error < 1e-9, \
            f"Energy error after 1 year: {rel_error:.2e}"
    ...
    def test_multiple_orbit_propagation(self):
        ...
        assert nu_diff_wrapped < np.deg2rad(0.1), \
